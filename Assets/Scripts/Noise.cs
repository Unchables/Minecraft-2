...
namespace SimplexNoise
{
    ...
    public static class Noise
    {
        ...
        [BurstCompile]
        public static float GetSimplexNoise(int x, int y, int z, float scale)
        {
        ...
        {
            Perm = new NativeArray<byte>(PermOriginal.Length, Allocator.Persistent);
            var random = new Unity.Mathematics.Random(seed);
            var temp = new NativeArray<byte>(PermOriginal.Length, Allocator.Temp);

            // 1. Fill a temporary array with ordered data (0-255)
            for(int i = 0; i < PermOriginal.Length; i++)
            {
                temp[i] = (byte)i;
            }

            // 2. Shuffle that array using the seeded random number generator
            for (int i = PermOriginal.Length - 1; i > 0; i--)
            {
            ...
            // 3. Copy the shuffled data into the final permutation table
            // (In the Burst solution, this is copied into the NativeArray)
            for(int i = 0; i < PermOriginal.Length; i++) Perm[i] = temp[i];
        }
        ...
        private static NativeArray<byte> Perm;
